...
/**
 * <p>{@link BeedPath} that selects a {@link PropertyBeed} from a
 *   {@link BeanBeed}. The {@link #getBeanBeed() bean beed} is the result of
 *   a {@link #getBeanBeedPath() bean beed path}. When the result of the
 *   {@link #getBeanBeedPath() bean beed path} changes, the result of this
 *   {@code BeanPropertyBeedPath} might change too. When this happens, this
 ...
 *   {link {@link #getBeanBeedPath()} (i.e., {@link #getBeanBeed()}).</p>
 *
 * @author Jan Dockx
 *
...
@CvsInfo(revision = "$Revision$",
         ...
         state    = "$State$",
         tag      = "$Name$")
public abstract class BeanPropertyBeedPath<_BeanBeed_ extends BeanBeed, _PropertyBeed_ extends PropertyBeed<?>>
    extends AbstractBeedPath<_PropertyBeed_> {
   ...
   */
  public BeanPropertyBeedPath(BeedPath<_BeanBeed_> beanBeedPath) {
    assert beanBeedPath != null;
    $beanBeedPath = beanBeedPath;
...
//    assert $beanBeedPath != null;
//    $dependent.removeUpdateSource($beanBeedPath);
//  }
  ...
  private final Dependent<BeedPath<_BeanBeed_>> $dependent =
    new AbstractUpdateSourceDependentDelegate<BeedPath<_BeanBeed_>, BeedPathEvent<_PropertyBeed_>>(this) {

      @Override
      protected BeedPathEvent<_PropertyBeed_> filteredUpdate(Map<BeedPath<_BeanBeed_>, Event> events, Edit<?> edit) {
        assert events != null;
        assert events.size() == 1;
        BeedPathEvent<_BeanBeed_> event = (BeedPathEvent<_BeanBeed_>)events.get($beanBeedPath);
        assert event != null;
        _PropertyBeed_ oldPropertyBeed = $propertyBeed;
        _BeanBeed_ newBeanBeed = event.getNewBeed();
        assert newBeanBeed == $beanBeedPath.get();
        $propertyBeed = newBeanBeed == null ? null : selectPropertyBeedFromBeanBeed(newBeanBeed);
        if (oldPropertyBeed != $propertyBeed) {
          return new BeedPathEvent<_PropertyBeed_>(BeanPropertyBeedPath.this, oldPropertyBeed, $propertyBeed, edit);
        }
        else {
          return null;
        }
   ...
   * @basic
   */
  public final BeedPath<_BeanBeed_> getBeanBeedPath() {
    return $beanBeedPath;
  }

  private final BeedPath<_BeanBeed_> $beanBeedPath;
