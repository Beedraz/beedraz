...
@CvsInfo(revision = "$Revision$",
         ...
         state    = "$State$",
         tag      = "$Name$")
public class CopyOfFilteredSetBeed<_Element_ extends Beed<_Event_>, _Event_ extends Event>
   ...
   * @post  get().isEmpty();
   */
  public CopyOfFilteredSetBeed(PathFactory<_Element_, BooleanBeed> criterion, AggregateBeed owner) {
    super(owner);
    $criterion = criterion;
  }
  ...
  private final Dependent $dependent = new AbstractUpdateSourceDependentDelegate(this) {
       ...
       * the {@link CopyOfFilteredSetBeed#getSourcePath()}. In this case, we
       ...
       * the {@link CopyOfFilteredSetBeed#getSource()}.
       ...
       * The elements that are added to {@link CopyOfFilteredSetBeed#$filteredSet}
       * by this operation are gathered in <code>addedFilteredElements</code>.
       * The elements that are removed from {@link CopyOfFilteredSetBeed#$filteredSet}
       ...
       * {@link CopyOfFilteredSetBeed#$filteredSet} and to <code>addedFilteredElements</code>.
       ...
       * {@link CopyOfFilteredSetBeed#$filteredSet} and add it to
       * <code>removedFilteredElements</code>.
       *
       * @pre  event != null;
       * @pre  addedFilteredElements != null;
      ...
      private void handleElementCriterionEvent(Event event,
        ...
        try {
          ...
          assert criterionEvent.getSource() instanceof CopyOfFilteredSetBeed.ElementCriterion;
   ...
   * @post    The {@link CopyOfFilteredSetBeed} is registered as a listener of the given SetBeed.
   * @post    The {@link CopyOfFilteredSetBeed} is registered as a listener of all beeds in
   *          the given source. (The reason is that the {@link CopyOfFilteredSetBeed} should be
  ...
  private ActualSetEvent<_Element_> createEvent(
    ...
    if ((! removedFilteredElements.isEmpty()) || (! addedFilteredElements.isEmpty())) {
      event = new ActualSetEvent<_Element_>(CopyOfFilteredSetBeed.this,
